...
EBAY_CREDENTIALS = {
    'client_id': os.getenv('SB_EBAY_CLIENT_ID'),
    'client_secret': os.getenv('SB_EBAY_CLIENT_SECRET'),
    'dev_id': os.getenv('SB_EBAY_DEV_ID'),
    'redirect_uri': os.getenv('SB_EBAY_REDIRECT_URI'),
    'business_policies': {
        'return': '245006369024',
...
def validate_payload(payload):
    ...
    required_fields = [
        "Title", "Description", "PrimaryCategory", "ConditionID", "Currency", "ListingType", "StartPrice", "Quantity", "Country", "Location", "ListingDuration", "BusinessPolicies", "ReturnPolicy", "ShippingDetails", "DispatchTimeMax", "ProductListingDetails", "ItemSpecifics"
    ]
    for field in required_fields:
        if field not in item or item[field] in [None, ""]:
            print(f"Missing or empty required field: {field}")
            return False
    nvl = item.get("ItemSpecifics", {}).get("NameValueList", [])
    if not isinstance(nvl, list):
        print("ItemSpecifics.NameValueList is not a list")
        return False
    for nv in nvl:
        if "Name" not in nv or "Value" not in nv or not isinstance(nv["Value"], list):
            print(f"Malformed NameValueList entry: {nv}")
            return False
    return True
...
def main():
    ...
    connection = Connection(
        debug=False,
        domain='api.ebay.com',  # Changed to sandbox
        config_file=None,
    ...
    for item in inventory:
        try:
            ...
            if not validate_payload(payload):
                print(f"Skipping item {item.get('id')} due to payload validation error. Payload:")
                print(payload)
                continue
            print(f"Submitting payload for item {item.get('id')}:\n{payload}")
            try:
                ...
                print(f"eBay API error for item {item.get('id')}: {str(api_e)}")
            time.sleep(1)
